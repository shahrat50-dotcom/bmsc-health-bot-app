      ...
      <div className="relative flex h-full w-full max-w-md flex-col overflow-hidden sm:border-x sm:border-sky-100 sm:shadow-2xl">
        ...
        <div className="absolute bottom-6 left-6 right-6">
          <nav className="bg-white rounded-[2.5rem] p-3 flex justify-between items-center shadow-2xl shadow-sky-900/20 border border-white overflow-x-auto gap-1">
            <NavItem to="/app" icon={(<svg className="w-5 h-5 mx-auto flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"/></svg>)} label={t('home')} />
            <NavItem to="/app/reminders" icon={(<Bell className="w-5 h-5 mx-auto flex-shrink-0" />)} label={t('reminders')} />
            <NavItem to="/app/scan" icon={(<svg className="w-6 h-6 mx-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01" /></svg>)} label={t('scan')} isCenter />
            <NavItem to="/app/chat" icon={(<svg className="w-5 h-5 mx-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>)} label={t('chat')} />
            <NavItem to="/app/history" icon={(<svg className="w-5 h-5 mx-auto flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>)} label={t('history')} />
          </nav>
        </div>
      </div>
    </div>
